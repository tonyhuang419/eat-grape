...
<form id="pagerForm" method="post" action="${ctx}/admin/system/useradmin/role/showIndex.htm?action=role_mgr&navTabId=${param.navTabId}">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="${ctx}/admin/system/useradmin/role/showAdd.htm?action=tjjszs&navTabId=${param.navTabId}" target="dialog" mask="true" width="520" height="250"><span>添加角色</span></a></li>
			<li><a class="delete" href="${ctx}/admin/system/useradmin/role/delete.htm?role.id={sid}&action=zxjssc&navTabId=${param.navTabId}" target="ajaxTodo" title="确定要删除吗?"><span>删除</span></a></li>
			<li><a class="edit" href="${ctx}/admin/system/useradmin/role/showUpdate.htm?role.id={sid}&action=xgjszs&navTabId=${param.navTabId}" target="dialog" mask="true" width="520" height="250"><span>修改角色</span></a></li>
		</ul>
	</div>
				...
				<tr target="sid" rel="${item.id}" align="center">
					...
					<td>
						<a title="${item.roleName}-设置权限" target="dialog" mask="true" width="780" height="480" 
							href="${ctx}/admin/system/useradmin/role/showSetPriv.htm?role.id=${item.id}&action=qxfpzs">设置权限</a>
						<a title="编辑" target="dialog" mask="true" width="520" height="250" 
							href="${ctx}/admin/system/useradmin/role/showUpdate.htm?role.id=${item.id}&action=xgjszs&navTabId=${param.navTabId}" class="btnEdit">编辑</a>
					</td>
				</tr>
			</s:iterator>
		</tbody>
