...
<form id="pagerForm" method="post" action="${ctx}/admin/foundation/business/feedback/showIndex.htm?action=city_community_area&navTabId=${param.navTabId}">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="${ctx}/admin/foundation/business/feedback/showIndex.htm?action=zxdyss&navTabId=${param.navTabId}" method="post">
	<div class="searchBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="delete" href="${ctx}/admin/foundation/business/feedback/delete.htm?feedback.id={sid}&action=zxdysc&navTabId=${param.navTabId}" target="ajaxTodo" title="确定要删除吗？"><span>删除信息</span></a></li>
			<li><a class="icon" href="${ctx}/admin/foundation/business/feedback/downXls.htm?fileName=Feedback.xls&action=dzzhexcel" target="dwzExport" targetType="navTab" title="确定要导出这些记录吗?"><span>导出信息</span></a></li>
		</ul>
	</div>
				...
				<tr target="sid" rel="<s:property value="#item.id" />" align="center">
					...
					<td>
						...
						<a class="delete" target="ajaxTodo" title="确定要删除吗？"
							href="${ctx}/admin/foundation/business/feedback/delete.htm?feedback.id=${item.id}&action=zxdysc&navTabId=${param.navTabId}" >
							删  除
						</a>&nbsp;&nbsp;&nbsp;
						<s:if test="#item.handleStatus != 2">
							<a title="反馈建议处理" target="dialog" rel="dialog_${item.id}" mask="false" minable="true" 
								href="${ctx}/admin/foundation/business/feedback/showHandle.htm?id=${item.id}&action=zdylbzs&navTabId=${param.navTabId}"
