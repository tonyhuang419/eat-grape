...
/******************************生成返回数字验证码图片*****************************************/
	 ...
	 * 生成数字验证码图片
	 * */
	public static BufferedImage getNumCheckCode(Color bgColor)
	{
		//创建缓冲区图形对象
		...
		//画干扰线
		CreateVerificationCode.drawRandomLine(g,4);
		
		//画随机数字
		String drawStr="0123456789";
		CreateVerificationCode.drawRandomChar((Graphics2D)g,drawStr,18,4);//将g其强制转换为2D绘图对象，以便实现随机字符旋转
...
/******************************私有方法*****************************************/
	...
	public static void method(Graphics g, Color bgColor)
	{
		//设置验证码图片背景颜色
		CreateVerificationCode.setBackground(g, bgColor);
		
		//设置验证码图片边框
		CreateVerificationCode.setBorder(g);
	}
...
//		g.draw3DRect(1, 1, WIDTH-2, HEIGHT-2,true);
	...
	private static void drawRandomChar(Graphics2D g,String drawStr,int fontSize,int drawCharCount)
	{
		...
		VerifyCodeAction.verifyCode_s = code.toString();//将生成的验证码值传给GetVerifyCode中得verifyCode_s
	}
}
